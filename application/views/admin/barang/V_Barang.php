...
<div class="content-wrapper">
    ...
    <div id="mHapusBarangGagal" class="modal fade">
        <div class="modal-dialog modal-confirm">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="icon-box" style="background-color: red;">
                ...
                <div class="modal-body">
                    <h4 class="modal-title" style="text-align: center;">Gagal Menghapus</h4>
                    </br>
                    <p class="text-center" style="color: red;">Harap Reload Browser Terlebih dahulu</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="mHapusStok" aria-hidden="true">
        <div class="modal-dialog modal-l" role="document">
            <form id="transaksi">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" style="text-align: center;">Peringatan !!!</h4>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-12 mb-3">
                                <hr>
                                <div class="form-control">
                                    <label class="form-label" style="font-size: 12pt;">Apakah anda ingin menghapus item
                                        ini?</label>
                                    <input hidden type="text" id="prodIdHapus" name="prodIdHapus"
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-danger" id="btn_HapusStockItem">Ok</button>
                    </div>
                </div>
            </form>
        </div>
...
$(document).ready(function() {
            ...
            },
            {
                "render": function(data, type, JsonResultRow, meta) {
                    return '<a title="Tambah Stock" id="tambahStock" class="open-AddBookDialog" data-id="' +
                        ...
                        '" data-bs-toggle="modal" data-bs-target="#mUpdateStok" href="#"> <i class="menu-icon tf-icons" style="text-align:center; color:grey"></i> <iconify-icon icon="fluent:box-arrow-up-24-regular" width="40" height="40"></iconify-icon> </a> <a title="Hapus Stock" id="hapusStock" class="open-AddBookDialog" data-id="' +
                        JsonResultRow.products_id +
                        '" data-bs-toggle="modal" data-bs-target="#mHapusStok" href="#"> <i class="menu-icon tf-icons" style="text-align:center; color:red"><iconify-icon icon="fluent:box-dismiss-24-regular" width="40" height="40"></iconify-icon></i>  </a>  ';
                }
    ...
    $('#btn_HapusStockItem').on('click', function() {
        var data = new FormData();
        data.append("prodIdHapus", $('#prodIdHapus').val());

        console.log(data);
        // console.log($('input[type=file]')[0].files[0]);
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>/submits_deactiveProd',
            dataType: "JSON",
            data: data,
            processData: false,
            contentType: false,
            success: function(data) {
                console.log(data);
                if (data.error) {
                    $('#mHapusStok').modal('hide');
                    $('#mHapusBarangGagal').modal('show');
                }
                if (data.success) {
    ...
    // $('#btn_deactiviedStock').on('click', function() {
    //     var data = new FormData();
    //     data.append("id_products", $('#id_products').val());
    // });

    $("#hapusStock").click(function() {
        var myId = $(this).data('id');
        $(".modal-body #prodIdHapus").val(myId);
        console.log(myId);
    });
